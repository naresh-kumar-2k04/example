...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
volatile uint8_t blinking = 0; // Shared between tasks
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  blinkSem = osSemaphoreNew(1, 0, &blinkSem_attributes); // Initial count = 0 (LED off)
  /* USER CODE END 2 */
...
void StartTask03(void *argument)
{
  ...
  /* Infinite loop */
	for (;;)
	    {
	        if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_RESET) // Button 1 pressed
	        {
	            blinking = 1; // Start blinking
	            while(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_RESET);
	        }

	        if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_1) == GPIO_PIN_RESET) // Button 2 pressed
	        {
